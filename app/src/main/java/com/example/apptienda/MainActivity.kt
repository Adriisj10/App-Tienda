...
class MainActivity : AppCompatActivity(), CategoriasAdapter.OnProductoListener {
    private lateinit var binding: ActivityMainBinding
    private lateinit var adapter: CategoriasAdapter //Objeto del adaptador
    private lateinit var listaProductos: ArrayList<ProductosJSON>
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val peticion: JsonObjectRequest = JsonObjectRequest("https://dummyjson.com/products", //Primer parametro lo que quiero consultr, en este caso la URL
            {  //Funcion por eso estan los corchetes, se trata cuando responde bien
                val array: JSONArray = it.getJSONArray("products") // creamos array del JSOn que es donde va a buscar los datos
                    for (i in 0..array.length()-1){ // Recorremos la array del JSON para encontrar los datos que queremos
                        ...
                        Log.d("RecyclerView", "Lista de productos en la app: ${listaProductos.size}")
                    }
    ...
    private fun instancias(){
        ...
        adapter = CategoriasAdapter(listaProductos,this)
        ...
        if (resources.configuration.orientation ==1) { //orientation 1 pantalla en vertical, por eso ponemos linear para que sea de uno en uno
            ...
            binding.recyclerProductos.layoutManager = GridLayoutManager(this,2, GridLayoutManager.HORIZONTAL,false)
        }
    }
    ...
    //llega el dato al MainActivity con los parametros que quiero comunicar, en este caso lo quiero en el Second Activity porque el producto va al carrito
    override fun onProductoSelected(producto: ProductosJSON) {
